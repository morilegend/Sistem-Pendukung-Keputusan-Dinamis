                        ...
                        <div class="mb-3 kriteria-row">
                            ...
                            <div class="d-flex align-items-center">
                                <select class="form-control sub-kriteria-select" data-kriteria-id="{{ $k->id }}"
                                    ...
                                    @empty
                                    <option value="">Sub-Kriteria Tidak Tersedia</option>
                                    @endforelse
                                </select>
                            </div>
                        </div>
...
$(document).ready(function() {
    $('#submitPenilaian').click(function() {
        ...
        $.ajax({
            url: "{{ route('pengguna.penilaian.store') }}",
            type: 'POST',
            data: {
                _token: "{{ csrf_token() }}",
            ...
            success: function(response) {
                window.location.href =
                    "{{ route('pengguna.penilaian.index', ['kategoriPenilaianId' => $kategoriPenilaian->id]) }}";
            },
            error: function(xhr) {
