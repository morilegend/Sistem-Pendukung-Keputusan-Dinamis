...
class AlternatifController extends Controller
{
    public function index(Request $request, $kategori_id)
    {
        ...
        if (!$kategori) {
            return redirect()->route('pengguna_utama.alternatif.index')
                ->with('error', 'Kategori Alternatif tidak ditemukan.');
        }
    
        $alternatif = Alternatif::where('id_kategori_alternatif', $kategori_id)
            ->get()
            ->map(function ($item) {
                $item->digunakanDalamPenilaian = \DB::table('penilaian')
                    ->where('id_alternatif', $item->id)
                    ->exists();
                return $item;
            });
    
        return view('Pengguna Utama.alternatif.alternatif', [
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('pengguna_utama.alternatif.index', ['kategori_id' => $request->id_kategori_alternatif])
    ...
    public function edit($id)
    {
        ...
        return view('Pengguna Utama.alternatif.edit', compact('alternatif', 'kategori'));
    }
    ...
    public function destroy($id)
    {
        ...
        // Redirect ke halaman alternatif dengan kategori_id
        return redirect()->route('pengguna_utama.alternatif.index', ['kategori_id' => $kategoriId])
